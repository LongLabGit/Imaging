function [tiffIDs,mTimes]=makeMovieTifDay(folder,binSize,groupOpt,Selectmotif,Motif,ind)

% This will make a movie tiff by averging across motifs

disp('Averaging them together to create one high resolution tiff') 
if ~exist([folder,'7-Full'],'dir')
    mkdir([folder,'7-Full'])
end
...
if isempty(Selectmotif)
    Selectmotif=1:length(Motif);
    newT_name=[folder,'7-Full\All',num2str(binSize,3),'day',ind,'.tif'];
else
    newT_name=[folder,'7-Full\All',num2str(binSize,3),'select.tif'];
end
...
for m=1:length(Selectmotif)
    origT_name=[pathloc,Motif(Selectmotif(m)).name];%the name/location of the tiff file
...
for c=1:length(cuts)-1
    ...
    for m=1:length(Selectmotif)
        tT=Motif(Selectmotif(m)).frameTimesWARP;%times of the tiffs in that motif. USE #4!!! this is the warped one
        ...
        if ~isempty(tiffs2take)
            for t2t=1:length(tiffs2take)
                ...
                ts(indImage,:)=[Selectmotif(m),tiffs2take(t2t)];%the motif and its index
    ...
    %now combine it
    if exist('Itemp','var')%we might have an empty bin
        switch groupOpt
            case 'avg'
    ...
    disp(['Progress=',num2str(c/(length(cuts)-1),3)])
end
%close them
for m=1:length(Selectmotif)
    TiffObjs(m).close();
