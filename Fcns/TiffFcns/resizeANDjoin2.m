function resizeANDjoin(folder,Motif,minLeft)
...
disp('Beginning to resize all the motifs and create the average tiff')
if ~exist([folder,'3a-MotifsMin\'],'dir')
    mkdir([folder,'3a-MotifsMin\'])
    mkdir([folder,'4-MotifsAvg\']);
end
...
for m=1:numM
    ...
    origT_obj.close();
    if sum(indReport==m)
        fprintf([num2str(m/numM,2),','])
    end
end
%get the minimum of each. this will inform 
w=max(min(wids),minLeft);%the minimum size to accept
l=max(min(lens),minLeft);
minSize=[l,w];
disp(['Done, min size=',num2str(minSize)])
avg_name=[f,'4-MotifsAvg\AVG.tif'];
...
for m=1:length(motifNames)
    ...
    currSize=[a,b];
    if sum(currSize>minSize)==2
        %first create a new set of cut motifs, where they are all the same size
        newT_name=[f,'3a-MotifsMin\',motifNames{m}];%the name/location of the tiff file
