function Motif=resizeANDjoin(folder,Motif,minLeft)
...
w=min(wids(wids>minLeft));%the minimum size to accept
l=min(lens(wids>minLeft));
...
for m=1:length(motifNames)
    ...
    if sum(currSize>minLeft)==2
        ...
        for i=1:Motif(m).numI
            ...
            if i==1
                Itemp=nan(size(imageData,1),size(imageData,2),Motif(m).numI);
...
Motif(tooMuchMotN)=[];
if ind>1
    fprintf('The following motifs are too small: ')
    disp(tooMuchMot)
end
totalT=toc;
