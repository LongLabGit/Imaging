...
maxNbursts = 5;%if we want to add bursts, whats the maximum bnumber that we will look for?
minNburst=1;
...
else%if we are trying to pass in paramters, rewrite the old ones
    for pn=1:length(paramNames)
...
% 
% if tau_max(1)<tau(1)
%     tau_max(1)=tau_max(1)+5;
% end
tau_max=[50,50];disp('changing tau max')
        ...
        for ti = 1:ntrials
            ...
            for ni = 1:N
                ...
                while tmp_a_>a_max || tmp_a_<a_min
                    ...
                    %if it gets stuck. take a look at taus
                    indA=indA+1;
                    if indA>1e4
                        keyboard;
                    end
