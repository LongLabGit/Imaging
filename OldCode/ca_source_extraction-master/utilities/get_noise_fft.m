 function [sn,psdx,ff] = get_noise_fft(Y,options)
        ...
        Fs = 1;        
        ff = 0:Fs/N:Fs/2;
        indf=ff>range_ff(1);      % cut out low frequencies
        indf(ff>range_ff(2))=0;   % cut out high frequencies so that we have a bandpass range we care about
        if dims > 1
            ...
            if ~split_data
                for ind = 1:ceil(d/Nb); 
                    ...
                    psdx = (1/(Fs*N)) * abs(xdft).^2; % take power
                    psdx(:,2:end-1) = 2*psdx(:,2:end-1); % double non-end components because FT is symmetric and we took only half
