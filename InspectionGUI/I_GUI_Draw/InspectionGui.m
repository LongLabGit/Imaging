...
% Last Modified by GUIDE v2.5 19-Sep-2017 15:24:47
...
% --- Executes on button press in draw3.
function draw3_Callback(hObject, eventdata, handles)
% hObject    handle to draw3 (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
drawROI(hObject,3);
plotROI(hObject,3);
...
handles.abf=uigetfile([handles.path '*.abf']);%get a list of the files 
guidata(hObject, handles);
pullupdata(hObject);
...
img=dir([handles.path,'\*.tif']);%get a list of the files 
answer = inputdlg('Max Shift (pixels)','maxShift',[1 40]);
maxShift=str2double(answer{1});
is.motionCorrect('savePath',[handles.path ,'\motC' img.name],'referenceFrame',1:10,'maxShift',maxShift);
handles.imgF=[handles.path ,'\motC' img.name];
% displayImg()
